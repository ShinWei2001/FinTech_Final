...
def download_single(ticker_base: str, start: str, end: str):
    """
    單檔下載，對含字母後綴的台灣 ETF（債券型）同時嘗試 .TW / .TWO
    回傳 (close_series, volume_series) 或 (None, None)
    """
...
def stage1_scoring(df100: pd.DataFrame, start: str, end: str) -> pd.DataFrame:
    """
    各類型分開評分：
      - 各類型內部各自計算 ADV / Active Risk / Cost 百分比排名
      - 各自產出 score_rank（類型內排名）
      - 合併後統一輸出，供 Stage 2 跨類型選最優10檔
    """
    all_cat_results = []
    ...
    for cat in ["stock", "bond", "other"]:
        ...
        print(f"  [{cat}] 逐檔下載 {len(tickers)} 檔...")
    ...
    scored["score_rank"] = scored.index + 1   # 跨類型全局排名
...
def stage2_elite_pool(scored: pd.DataFrame) -> pd.DataFrame:
    ...
    調整策略：
      - 約束 1/2 不足：從該類型補入分數最高的候選，踢掉末位 stock
      - 約束 3 超過：把超出的 bond/other（從分數最低的開始）換回 stock
    """
    ...
    def fill_from(cat):
        """從 cat 類型補入一檔分數最高的非在池候選，踢掉末位 stock"""
        nonlocal top10
    ...
    def trim_nonstock():
        """bond+other > 4 時，把分數最低的 bond/other 換回分數最高的 stock"""
        nonlocal top10
