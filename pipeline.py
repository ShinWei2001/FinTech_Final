...
Stage 1  量化評分（各類型內部比較）
  - 股票型/其他型：ADV + Active Risk + Cost
  - 債券型：市值代理ADV + 固定中位分 + Cost
  - 類型內百分比排名，跨類型合併後統一排序
...
輸出額外欄位 all_ticker_info + price_data，供前端互動重算
"""
...
# 全部 318 檔的靜態資訊 lookup（ticker → {category, cost, mktcap}）
ALL_INFO = {}
...
def stage0_market_cap_filter():
    ...
    for cat, lst in cats.items():
        for i, (ticker, mktcap, cost) in enumerate(lst[:floored[cat]]):
            selected.append({"ticker": ticker, "category": cat,
...
def stage1_scoring(df100: pd.DataFrame, start: str, end: str) -> pd.DataFrame:
    ...
    for cat in ["stock", "bond", "other"]:
        ...
        if cat == "bond":
            # 債券型：靜態指標（Yahoo Finance 不支援字母後綴代碼）
            rows = []
            for _, row in sub.iterrows():
                rows.append({"ticker": row["ticker"], "category": cat,
                              "mktcap": row["mktcap"], "mktcap_rank": row["mktcap_rank"],
                              "cost": row["cost"], "ADV": row["mktcap"], "active_risk": None})
            cat_df = pd.DataFrame(rows)
            cat_df["score_adv"]  = score_pct_rank(cat_df["ADV"], higher_is_better=True)
            cat_df["score_risk"] = 50.0
            ...
            for _, row in sub.iterrows():
                ...
                rows.append({"ticker": t, "category": cat,
                              "mktcap": row["mktcap"], "mktcap_rank": row["mktcap_rank"],
        ...
        cat_df["total_score"] = (cat_df["score_adv"] + cat_df["score_risk"] + cat_df["score_cost"]) / 3
    ...
    if not all_results:
        raise RuntimeError("Stage 1 無任何有效資料")
...
def stage2_elite_pool(scored: pd.DataFrame) -> pd.DataFrame:
    ...
    def fill_from(cat):
        nonlocal top10
        cands = scored[(scored["category"] == cat) & (~scored["ticker"].isin(top10["ticker"]))]
        if cands.empty:
        ...
        if stocks.empty:
            return
        top10.drop(index=stocks.index[-1], inplace=True)
        top10.__init__(pd.concat([top10, cands.head(1)]).sort_values("total_score", ascending=False).reset_index(drop=True))
    ...
    def trim():
        ...
        while count("bond") + count("other") > 4:
            ...
            cands  = scored[(scored["category"] == "stock") & (~scored["ticker"].isin(top10["ticker"]))]
            if cands.empty:
                break
            top10 = pd.concat([top10[top10["ticker"] != drop_t], cands.head(1)]).sort_values("total_score", ascending=False).reset_index(drop=True)
...
def build_mvo_tickers(elite: pd.DataFrame, scored: pd.DataFrame,
    ...
    """確保 MVO 有 target 檔可下載的標的；抓不到的從同類型補位，最後用股票型補"""
    ...
    for t in pool:
        if can_download(t, start, end):
            ...
            filled = False
            for _, row in cands.sort_values("total_score", ascending=False).iterrows():
                if can_download(row["ticker"], start, end):
                    result.append(row["ticker"]); used.add(row["ticker"])
                    print(f"    → 補入 {row['ticker']} ({cat})")
                    filled = True; break
            if not filled:
                fb = scored[(scored["category"] == "stock") &
                            (~scored["ticker"].isin(used)) &
                            (~scored["ticker"].isin(pool))]
                for _, row in fb.sort_values("total_score", ascending=False).iterrows():
                    if can_download(row["ticker"], start, end):
                        result.append(row["ticker"]); used.add(row["ticker"])
...
def backtest(smart_weights: dict, all100_tickers: list,
    ...
    if sc is None or ec is None: return {}
    ...
    return {
        ...
        "metrics": {
            "smart_e":         {"cumulative_return": round(float(sc.iloc[-1]-1),4), "max_drawdown": round(sm,4), "sharpe": round(ss,4)},
            "equal_weight_100":{"cumulative_return": round(float(ec.iloc[-1]-1),4), "max_drawdown": round(em,4), "sharpe": round(es,4)},
        },
    }


# ══════════════════════════════════════════════════════════════
# Price Data（供前端互動重算 MVO）
# ══════════════════════════════════════════════════════════════

def build_price_data(tickers: list, start: str, end: str) -> dict:
    """
    輸出各標的正規化收盤價序列（第一天=1.0），供前端 Monte Carlo 用
    純數字代碼用批次下載加速；含字母後綴（債券/其他型）逐檔嘗試 .TW/.TWO
    """
    result = {}

    # 分兩組：純數字（可批次）vs 含字母（只能逐檔）
...
def main():
    ...
    if mvo and mvo.get("weights"):
        ...
        if bt:
            ...
            print(f"  Smart-E    {sm['cumulative_return']:.1%}  MDD {sm['max_drawdown']:.1%}  Sharpe {sm['sharpe']:.2f}")
            print(f"  Equal(100) {eq['cumulative_return']:.1%}  MDD {eq['max_drawdown']:.1%}  Sharpe {eq['sharpe']:.2f}")

    # Price data（全部 318 檔，涵蓋投資人可能自選的任意台灣 ETF）
    print("\n【Price Data】下載全部 318 檔供前端使用（債券型字母後綴代碼可能較多失敗）...")
    all318_tickers = list(ALL_INFO.keys())
    price_data = build_price_data(all318_tickers, start_date, end_date)
    print(f"  成功 {len(price_data)}/{len(all318_tickers)} 檔")

    # all_ticker_info：所有 MVO 標的的完整靜態資訊
    all_ticker_info = {
        t: {
    ...
    output = {
        ...
        "stage2": {
            "elite_pool": elite[[
                "ticker","category","mktcap","cost","total_score","score_rank","pool_rank"
            ]].round(3).to_dict("records"),
            "all_ticker_info": all_ticker_info,   # ← 所有 MVO 標的靜態資訊
            "mvo": mvo,
        },
        "backtest":   bt,
        "price_data": price_data,                 # ← 供前端互動重算
    }
