...
def stage1_scoring(df100: pd.DataFrame, start: str, end: str) -> pd.DataFrame:
    ...
    for cat in ["stock", "bond", "other"]:
        ...
        print(f"  [{cat}] 下載 {len(tickers)} 檔價格資料（分批，逐檔重試）...")
        close_frames, vol_frames = [], []
        for i in range(0, len(tw_tickers), 20):
            chunk = tw_tickers[i:i+20]
            try:
                ...
                if isinstance(raw.columns, pd.MultiIndex):
                    c = raw["Close"].copy(); v = raw["Volume"].copy()
                else:
                    c = raw[["Close"]].copy(); v = raw[["Volume"]].copy()
                    c.columns = v.columns = [chunk[0]]
                c.columns = [x.replace(".TW","") for x in c.columns]
                v.columns = [x.replace(".TW","") for x in v.columns]
                close_frames.append(c); vol_frames.append(v)
            except Exception:
                # 批次失敗時改成逐檔單獨下載（對債券型字母後綴代碼更穩定）
                for t in chunk:
                    ticker_clean = t.replace(".TW", "")
                    # 嘗試 .TW 和 .TWO 兩種後綴（債券型帶字母代碼在上櫃用 .TWO）
                    base = ticker_clean
                    for fmt in [f"{base}.TW", f"{base}.TWO", f"{base}.TW".upper()]:
                        try:
                            r = yf.download(fmt, start=start, end=end,
                                            auto_adjust=True, progress=False)
                            if r.empty or len(r) < 30:
                                continue
                            if isinstance(r.columns, pd.MultiIndex):
                                r = r["Close"].to_frame()
                            c = r[["Close"]].rename(columns={"Close": base}) if "Close" in r.columns else r.iloc[:,:1].rename(columns={r.columns[0]: base})
                            vol_col = r[["Volume"]].rename(columns={"Volume": base}) if "Volume" in r.columns else pd.DataFrame(index=r.index)
                            close_frames.append(c); vol_frames.append(vol_col)
                            print(f"    {base}: OK via {fmt}")
                            break
                        except Exception:
                            continue
        if not close_frames:
            print(f"  [{cat}] 無任何有效資料，略過此類型")
            continue
        close  = pd.concat(close_frames, axis=1).ffill().bfill()
        volume = pd.concat(vol_frames,   axis=1).ffill().bfill() if vol_frames else pd.DataFrame()
    ...
    # ── 各類型內部各自做百分比排名（避免跨類型 ADV 尺度差異造成債券型全部墊底）
    for col, higher in [("ADV", True), ("active_risk", False), ("cost", False)]:
        score_col = f"score_{col.split('_')[0] if '_' in col else col.lower()}"
        if col == "active_risk":
            score_col = "score_risk"
        elif col == "ADV":
            score_col = "score_adv"
        elif col == "cost":
            score_col = "score_cost"
        scored[score_col] = 0.0
        for cat in ["stock", "bond", "other"]:
            mask = scored["category"] == cat
            if mask.sum() == 0:
                continue
            scored.loc[mask, score_col] = score_pct_rank(
                scored.loc[mask, col], higher_is_better=higher
            ).values
...
# ══════════════════════════════════════════════════════════════
# STAGE 2：優選池（前 10 檔，債券 ≥ 1 且其他 ≥ 1）→ MVO
# ══════════════════════════════════════════════════════════════

def stage2_elite_pool(scored: pd.DataFrame) -> pd.DataFrame:
    """
    取總分前 10 檔（即前 10%）
    硬約束：bond >= 1 且 other >= 1（各自獨立保證）
    做法：
      1. 先從前 10 確認 bond/other 是否各自達標
      2. 哪個類型不足，就從該類型補入分數最高且尚未入選的那一檔
      3. 補進來的每一檔，踢掉當時排名最末的 stock
    """
    top10 = scored.head(10).copy()

    for cat in ["bond", "other"]:
        if (top10["category"] == cat).sum() < 1:
            in_pool = set(top10["ticker"])
            candidate = scored[
                (scored["category"] == cat) &
            ...
            if candidate.empty:
                print(f"  Warning: {cat} no candidate available, skipping constraint")
                continue

            stock_rows = top10[top10["category"] == "stock"]
            if stock_rows.empty:
                print(f"  Warning: no stock to replace for {cat}, skipping")
                continue
            drop_idx = stock_rows.index[-1:]
