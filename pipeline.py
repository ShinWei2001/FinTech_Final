...
def fetch_twse_adv(ticker: str, months: int = 3) -> float:
    """
    從 TWSE openapi 抓近 N 個月每日成交量，計算平均日成交量
    用於 Yahoo Finance 抓不到的債券型 ETF
    回傳平均日成交張數（float），失敗回傳 NaN
    """
...
def stage1_scoring(df100: pd.DataFrame, start: str, end: str) -> pd.DataFrame:
    ...
    for cat in ["stock", "bond", "other"]:
        ...
        if cat == "bond":
            # ── 債券型：TWSE API 抓 ADV，Active Risk 給固定中位分
            results = []
            for _, row in sub.iterrows():
                t   = row["ticker"]
                adv = fetch_twse_adv(t, months=3)
                results.append({
                    "ticker":      t,
                    "category":    cat,
                    "mktcap":      row["mktcap"],
                    ...
                    "cost":        row["cost"],
                    "ADV":         adv,
                    "active_risk": np.nan,   # 債券型不計算 active risk
                })

            cat_df = pd.DataFrame(results)
            cat_df = cat_df.dropna(subset=["ADV"])   # 只需 ADV 有值
            if cat_df.empty:
                print(f"  [{cat}] TWSE API 無法取得任何資料，略過")
                continue

            cat_df["score_adv"]  = score_pct_rank(cat_df["ADV"],  higher_is_better=True)
            cat_df["score_risk"] = 50.0   # 債券型 active risk 統一給中位分
